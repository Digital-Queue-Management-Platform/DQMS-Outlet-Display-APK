# DQMP Outlet Display - Professional Android TV App
...
- **Audio Queue Management**: Handles multiple announcements with priority
...
- **Enter/OK**: Select items and confirm actions
- **Back**: Return to previous screen or dismiss dialogs

### Professional Access Codes
...
1. **Install APK**: Install `DQMP_Outlet_Display_V1.apk` on Android TV device
2. **Launch App**: App will show QR code setup screen on first run
3. **Scan QR Code**: Use teleshop manager dashboard to scan the QR code
4. **Automatic Configuration**: Display will automatically configure and start

### QR Code Setup Process

1. **Display Shows QR Code**: Unique QR code displayed on TV screen
2. **Manager Scans Code**: Teleshop manager scans QR from dashboard
3. **Outlet Assignment**: Manager assigns outlet ID and server configuration  
...
- **Customer Names**: "John, token number 105, please proceed to counter 3"
- **Recall Announcements**: "Token number 105 is being recalled"
...
- **Up Next Queue**: Scrolling list of waiting customers
- **Service Type Badges**: Color-coded service category indicators
- **Counter Status Panel**: Real-time officer status and availability
...
- **Kiosk Mode**: Optionally locks device to only show DQMP display
- **24/7 Operation**: Designed for continuous retail operation
- **Crash Recovery**: Automatically restarts if issues occur
- **Fleet Management**: Supports deployment across multiple locations

### Security & Reliability
- **Encrypted Configuration**: All settings encrypted with device keys
- **Offline Capability**: Continues operation during network outages
...
- **Android TV Boxes**: Works on all standard Android TV boxes
- **Smart TVs**: Compatible with Android TV-powered smart televisions

### Tech Stack
- **Kotlin & Jetpack Compose for TV**
- **Modern Architecture**: MVVM with Kotlin Coroutines
- **Professional Libraries**: Material 3, DataStore, Retrofit
- **Real-time Updates**: WebSocket with HTTP polling fallback
- **Enterprise Security**: Encrypted SharedPreferences, secure storage
...
**Perfect for**: Banks, government offices, retail stores, service centers, hospitals, and any business requiring professional queue management displays.

---

*DQMP Outlet Display - Professional Android TV Queue Management System*
